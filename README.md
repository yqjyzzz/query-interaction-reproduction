...
- `.github/workflows/quality.yml` 在干净环境中执行同一套仓库检查；
- 生成目录、缓存和本地权重不进入版本控制。
...
论文引用信息见 [`CITATION.cff`](CITATION.cff)。仓库级许可尚待作者最终确认，当前状态见 [`LICENSE_STATUS.md`](LICENSE_STATUS.md)。数据集、模型与第三方代码继续遵循各自的原始许可。
